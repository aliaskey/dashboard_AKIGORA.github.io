...
###################################################################
        ...
        # # Calculer les titres d'intervention les plus fréquents
        # intitules_populaires = df_intervention['intitule'].value_counts().head(10)  # Afficher les 10 premiers
        # # Supposons que df_expert contient une colonne 'profile_views'
        # nb_consultations_profils = df_expert['profile_views'].sum()
        # nb_abonnes_newsletter = df_newsletter[df_newsletter['subscribed'] == True].shape[0]
        # pourcentage_abonnes_newsletter = (nb_abonnes_newsletter / df_user.shape[0]) * 100
        # # Supposons que df_expert contient une colonne 'legal_status'
        # repartition_statut_juridique = df_expert['legal_status'].value_counts(normalize=True) * 100
        # st.bar_chart(repartition_statut_juridique)
        # # Supposons que df_expert contient une colonne 'years_experience'
        # repartition_annees_experience = df_expert['years_experience'].value_counts(normalize=True) * 100
        # st.bar_chart(repartition_annees_experience)
        # moyenne_annees_experience = df_expert['years_experience'].mean()
   
        #     # Affichage des KPI dans Streamlit
        # st.markdown("<h3 style='text-align: center;'>KPI du département des ressources humaines</h3>", unsafe_allow_html=True)
        # col1, col2, col3, col4 = st.columns(4)
        # with col1:
        #     st.metric("Titres d'Intervention les Plus Fréquents", intitules_populaires)
        # with col2:
        #     st.metric("Nombre de Consultations de Profils", nb_consultations_profils)
        # with col3:
        #     st.metric("Pourcentage d'Abonnés Newsletter", f"{pourcentage_abonnes_newsletter:.2f}%")
        # with col4:
        #     st.metric("Nombre Moyen d'Années d'Expérience", f"{moyenne_annees_experience:.2f}")
###################################################################################################""""                   
#onglet commerce    
    with tab3:
